    ...
    DC.cart.Cart = {
        ...
        init: function(options) {
            ...
            if (this.language) {
                ...
                this._widget.addClass("cartWidget button");
            ...
            this._container = new DC.cart.ContatinerView({
                ...
                onRightButtonClick: function() {
                    if (self._currentSection == "cart") {
                        if (self._cartController.needShipping) {
                    ...
                    } else if (self._currentSection == "address") {
                            ...
                            //send ajax form
                            $('#DEFAULT_NAME_SUBMIT_BUTTON').submit( function(e) {  
                                $.ajax({
                    ...
                    } else if (self._currentSection == "thanks") {
                    	self._changeSection("cart");
                        self.closeCart();
                    	//this._cartController.onCheckout(this._cartController);
            ...
            this._thanksView = new DC.cart.ThanksView();
        ...
        _updateCartView: function(cart) {
            ...
            if (this._widget) {
                var content = $('<div class="amount">' + DC.Currency[cart.currency] + cart.subTotal.toFixed(2) + '</div><div class="badge">' + DC.cart.lang["cartwidget"].replace("%S", cart.count) + "</div>");
                content.hide();
                this._widget.empty();
        ...
        _changeSection: function(section) {
            if (section == "cart") {
                ...
                this._container.leftButton.setVisible(false);
                this._container.setSection(1);
                    ...
                    this._container.leftButton.setVisible(true);
            ...
            } else if (section == "thanks") {
                	this._container.setContent(this._thanksView);
                    //var shipping_info = DC.cart.lang["label_shipping_info"].replace("%S", DC.Currency[this._cartController.currency] + this._cartController.shipping);
                    this._container.setTitle("thank you.");
                    this._container.leftButton.setVisible(false);
                    this._container.rightButton.setVisible(true);
                    this._container.setSection(4);
            }
            ...
            if (this._cartController) {
                if (this._currentSection == "cart") {
                    ...
                    if (this._cartController.needShipping) {
                        this._container.rightButton.setText(DC.cart.lang["button_shipping"]);
                    } else {
                        this._container.rightButton.setText(DC.cart.lang["button_billing"]);
                    }
                } else {
                    if (this._currentSection == "address") {
                        ...
                        if (this._currentSection == "payment") {
                            if (this._cartController.needShipping) {
                                this._container.leftButton.setText(DC.cart.lang["button_shipping"]);
                            } else {
                                this._container.leftButton.setText(DC.cart.lang["button_cart"]);
                            }
                        
                    }else {
                    	this._container.rightButton.setText(DC.cart.lang["button_finish"]);
                    }
                }
            }
        },
        _updateStyles: function() {
            ...
            if (this.themeColor) {
                ...
                //$(".cartWidget").css("box-shadow", "0px 0px 6px "+this.themeColor);
            }
...
(function(window) {
    ...
    DC.cart.CartController = {
        ...
        getPaypalButton: function() {
            ...
            for (var p in this.paypalOptions) {
                if (p != "server") {
                    address += '<input type="hidden" name="' + p + '" value="' + this.paypalOptions[p] + '">';
                }
            }
            if (this.paypalOptions.server == undefined) {
                this.paypalOptions.server = "product/checkout";
            }
            var target = "";
            if (this.openNewCheckOutWindow) {
                target = 'target="_blank"';
            ...
            var id = this.id + "_SUBMIT_BUTTON";
            var button = $('<form id="' + id + '" action="' + this.paypalOptions.server + '" method="post" ' + target + '><input type="hidden" name="cmd" value="_cart"> <input type="hidden" name="securityToken" value="'+ip.securityToken+'"> <input type="hidden" name="upload" value="1"> <input type="hidden" name="business" value="' + this.paypalOptions.business + '">' + items + address + '</form>');
            button.click(function(e) {
                $("#" + id).trigger("submit");
                self.onCheckout(self);
            });
    ...
    DC.cart.ContatinerView = {
        ...
        setSize: function(w, h) {
            ...
            if (this._haveAddress) {
                this._headerWidth = (this.innerWidth-30) / 3;
            } else {
                this._headerWidth = (this.innerWidth-30) / 2;
            }
        ...
        setNeedShipping: function(address) {
            ...
            this._headerContainer.addChild(new DC.Button({
                styles: "righttop",
                text: "X",
                onClick: function() {
                    cart.closeCart();
    ...
    DC.cart.CartView = {
        ...
        update: function(cart) {
            ...
            if (cart.count > 0) {
                ...
                this.totalLabel.setText(DC.cart.lang["label_total"] + ' <div class="currency">' + DC.Currency[cart.currency] + '</div><div class="amount">' + cart.subTotal.toFixed(2) + "</div>");
...
(function(window) {
    ...
    window.DC.cart.ThanksView = DC.Container.extend(DC.cart.ThanksView);
}(window));
(function(window) {
    DC.cart.PaymentView = {
        ...
        updatePayment: function(cart) {
            var detail = DC.cart.lang["label_item_cost"].replace("%S", cart.count).replace("%A", DC.Currency[cart.currency] + cart.subTotal) + "<br/>";
            if (cart.needShipping) {
                detail += DC.cart.lang["label_shipping_cost"].replace("%S", DC.Currency[cart.currency] + cart.shipping) + "<br/>";
            }
            
            detail += DC.cart.lang["label_total_cost"].replace("%S", DC.Currency[cart.currency] + cart.total) + "<br/><br/>";

            detail += "親愛的 " +cart.address.firstname+' ' +cart.address.lastname + "請確認信息是否正確<br/>";
            detail += "address:" +cart.address.line1 + "<br/>";
            detail += "email:" +cart.address.line2 + "<br/>";
            detail += "phone:" +cart.address.phone + "<br/>";
           
            for(var item in cart.items){
            	detail += cart.items[item].name +"\tx\t"+ cart.items[item].qty + "<br/>";
            }
    ...
    $.fn.extend({
        DCAJAXPaypalCart: function(options) {
            ...
            return this.each(function() {
                options.widget = self;
                //options.footer = '<img src="' + DC.LOGO + '" align="right"/><div style="font-size: 0.7em;">AJAX Paypal Cart (Free Version). Develop by <a href="http://www.digicrafts.com.hk/components">Digicrafts.</a></div>';
